...
public class BookController {
    ...
    /**
     * GET /api/books/recent - Get recently published books (top 5)
     */
    @GetMapping("/recent")
    public ResponseEntity<List<Book>> getRecentBooks() {
        try {
            ...
            System.err.println("Error getting recent books: " + e.getMessage());
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
        }
    }
    ...
    /**
     * GET /api/books/stats/genre/{genre} - Get count of books by genre
     */
    @GetMapping("/stats/genre/{genre}")
    public ResponseEntity<Long> getBookCountByGenre(@PathVariable String genre) {
        try {
            long count = bookService.countBooksByGenre(genre);
            return ResponseEntity.ok(count);

        } catch (Exception e) {
            System.err.println("Error getting book count by genre: " + e.getMessage());
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
        }
    }
    }
